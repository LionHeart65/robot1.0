...
@TeleOp(name = "Red Pedro Teleop")
public class RedPedroTeleop extends LinearOpMode {
    ...
    private final Pose startPose = new Pose(9,33,-Math.PI/2);
...
//                        odo.reset();
    ...
    private void setAutoMove(Pose endPos, Pose controlPoint) {
        Path path;
        SparkFunOTOS.Pose2D robotPos = odo.getPos();

        path = new Path(new BezierCurve(new Point(new Pose(robotPos.x, robotPos.y, robotPos.h)), new Point(controlPoint), new Point(endPos)));
        path.setLinearHeadingInterpolation(odo.getPos().h, endPos.getHeading());
